...
import { PettyCashShowProps } from "./types";
...
function PettyCashShow({ pettyCash, accounts, flash }: PettyCashShowProps) {
    ...
    const handleConfirmApprove = useCallback(
        async (sourceAccountId: number) => {
            setIsApproving(true);
            router.post(
                route("petty-cashes.approve", pettyCash.id),
                { sourceAccountId },
                {
                    preserveScroll: true,
    ...
    const handleConfirmReject = useCallback(
        async (reason: string) => {
            setIsRejecting(true);
            router.post(
                route("petty-cashes.reject", pettyCash.id),
                { reason },
                {
                    preserveScroll: true,
    ...
    const statusInfo = getStatusInfo(pettyCash.status);

    return (
        <>
            <Head title={`Detail Kas Kecil - ${pettyCash.code}`} />
                ...
                <div className=" mx-auto space-y-6">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className="p-3 rounded-xl bg-[var(--color-primary-100)]">
                                <Wallet className="w-8 h-8 text-[var(--color-primary-600)]" />
                            </div>
                            <div>
                                <div className="flex items-center gap-3">
                                    <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">
                                        Detail Permintaan Kas Kecil
                                    </h1>
                                    <Badge
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <Card className="p-6">
                                ...
                                <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-4 rounded-xl bg-[var(--color-surface-hover)] border border-[var(--color-border)]">
                                    <div className="flex-1 text-center md:text-left">
                                        ...
                                        <p className="font-bold text-[var(--color-text-primary)]">
                                            {pettyCash.sourceAccount?.name ||
                                                (pettyCash.status === "pending"
                                                    ? "Pilih Akun Sumber..."
                                                    : "-")}
                                        </p>
                                        <p className="text-xs text-[var(--color-text-tertiary)]">
                                            {pettyCash.sourceAccount?.code ||
                                                (pettyCash.status === "pending"
                                                    ? "Ditentukan saat approval"
                                                    : "-")}
                                        </p>
                                    </div>
                                ...
                                {pettyCash.journalEntry && (
                                    <div className="mt-4 p-3 rounded-lg bg-[var(--color-info-50)] border border-[var(--color-info-100)] text-[var(--color-info-700)] text-xs flex gap-2 items-start">
                                        ...
                                        <span>
                                            Permintaan ini telah dicatat dalam
                        ...
                        <div className="space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center text-[var(--color-text-secondary)]">
                                            ...
                                            <span className="font-semibold text-[var(--color-text-primary)]">
                                                {pettyCash.requestDateFormatted ||
                                                    "-"}
                                            </span>
                                        </div>
                                        ...
                                        {pettyCash.approvedByUser && (
                                            <div className="flex justify-between items-center text-[var(--color-text-secondary)] pt-2 border-t border-black/5">
                            ...
                            {/* Info Card */}
                            <Card className="p-6 bg-gradient-to-br from-[var(--color-primary-500)]/5 to-transparent border-none shadow-none">
                                <div className="flex items-center gap-2 text-[var(--color-primary-600)] mb-2 font-bold uppercase tracking-wider text-xs">
                                    <TrendingUp className="w-3 h-3" />
                                    Informasi Kas Kecil
                                </div>
                                <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed">
                                    Kas kecil digunakan oleh kasir outlet untuk
                                    keperluan operasional mendesak atau biaya
                                    kecil lainnya. Setiap pengisian harus
                                    mendapatkan persetujuan owner dan akan
                                    mengurangi saldo akun sumber yang Anda
                                    pilih.
                                </p>
                            </Card>
                        </div>
...
PettyCashShow.layout = withAuthenticatedLayout({
    title: "Detail Kas Kecil",
    breadcrumbs: [
        { label: "Kas Kecil", href: route("petty-cashes.index") },
        { label: "Detail", href: "#" },
    ],
});
