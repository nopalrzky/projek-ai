...
abstract class PrintRemoteDatasource {
  ...
  Future<PrintCoinInfoModel> processReceipt(int orderId);
  Future<PrintCoinInfoModel> processLabel(int orderId);
}
...
class PrintRemoteDatasourceImpl implements PrintRemoteDatasource {
  ...
  @override
  Future<PrintCoinInfoModel> processReceipt(int orderId) async {
    try {
      final response = await _dio.post(_endpoints.orderPrintReceipt(orderId));
      final body = validateResponse(response);
  ...
  @override
  Future<PrintCoinInfoModel> processLabel(int orderId) async {
    try {
      final response = await _dio.post(_endpoints.orderPrintLabel(orderId));
      final body = validateResponse(response);
      return parsePrintCoinInfo(body, response.statusCode);
    } catch (e) {
      throw _handleError(e);
