...
class CartCubit extends Cubit<CartState> {
  ...
  Future<void> loadCart() async {
    ...
    if (cartJson != null) {
      try {
        ...
        final List<dynamic>? servicesList = decoded['activeServices'] as List<dynamic>?;
        final List<dynamic>? nonCourierServicesList =
            decoded['nonCourierServiceIds'] as List<dynamic>?;
        
        final Set<int> activeServices = servicesList?.map((e) => e as int).toSet() ?? {};
        final Set<int> nonCourierServiceIds = nonCourierServicesList?.map((e) => e as int).toSet() ?? {};
  ...
  void setActiveOutlet(int outletId, String outletName) {
    emit(state.copyWith(activeOutletId: outletId, activeOutletName: outletName));
  }
  ...
  Future<AddServiceResult> addService(
    ...
    if (currentState.hasItems && currentState.activeOutletId != null && currentState.activeOutletId != outletId) {
      return AddServiceResult.conflictOutlet;
    }

    if (currentState.activeServices.contains(serviceId) && currentState.activeOutletId == outletId) {
  ...
  Future<void> removeFromCart(int serviceId) async {
    final currentState = state;
    final newServices = Set<int>.from(currentState.activeServices)..remove(serviceId);
    final newNonCourierIds = Set<int>.from(currentState.nonCourierServiceIds)..remove(serviceId);
  ...
  Future<void> clearCart() async {
    ...
    await _saveCart(state.activeOutletId, state.activeOutletName, const {}, const {});
  }
